...
class DeepSeekGUI:
    ...
    def _process_message_thread(self, message):
        ...
        timeout_timer = threading.Timer(60.0, self._timeout_callback)
        timeout_timer.start()
    ...
    def _timeout_callback(self):
        ...
        self.root.after(0, lambda: self.add_to_chat("❌ Request timed out after 60 seconds. Please try again.", 'error'))
        self.root.after(0, lambda: self.send_button.config(state=tk.NORMAL))
        self.root.after(0, lambda: self.update_status("Ready"))

    def stream_openai_response_gui(self, user_message: str):
        """Properly handle streaming response for GUI"""
        ...
        try:
            ...
            self._schedule_add_chat("🐋 Seeking...", 'function')
            ...
            for chunk in stream:
                # Handle reasoning if available (some models may not have this)
                if hasattr(chunk.choices[0].delta, 'reasoning') and chunk.choices[0].delta.reasoning:
                    if not reasoning_started:
                        reasoning_started = True
                        self._schedule_add_chat("💭 Reasoning:", 'reasoning')

                    reasoning_chunk = chunk.choices[0].delta.reasoning
                    reasoning_content += reasoning_chunk
                    # Update reasoning display in real-time
                    self._schedule_append_message(reasoning_chunk, 'reasoning')

                if chunk.choices[0].delta.content:
                    if not final_content:  # First content chunk
                        self._schedule_add_chat("🤖 Assistant:", 'assistant')

                    content_chunk = chunk.choices[0].delta.content
                    final_content += content_chunk
                    assistant_message["content"] += content_chunk
                    # Update content display in real-time
                    self._schedule_append_message(content_chunk, 'assistant')
            ...
            if tool_calls:
                ...
                self._schedule_add_chat(f"⚡ Executing {len(tool_calls)} function call(s)...", 'function')

                for tool_call in tool_calls:
                    function_name = tool_call["function"]["name"]
                    self._schedule_add_chat(f"→ {function_name}", 'function')
                    ...
                    try:
                        ...
                        if "Successfully" in result or "Content of file" in result:
                            success_msg = f"✓ {result.split(':')[0] if ':' in result else result}"
                            self._schedule_add_chat(success_msg, 'success')
                        else:
                            self._schedule_add_chat(f"ℹ️ {result}", 'function')

                    except Exception as e:
                        error_msg = f"Error executing {function_name}: {e}"
                        self._schedule_add_chat(f"✗ {error_msg}", 'error')
                ...
                self._schedule_add_chat("🔄 Processing results...", 'function')
    ...
    def _schedule_append_message(self, text: str, tag: str):
        """Schedule appending text to chat in main thread"""
        # Create a proper closure to capture the current values
        def append_text():
            self._append_to_last_message(text, tag)
        self.root.after(0, append_text)

    def _schedule_add_chat(self, message: str, tag: str):
        """Schedule adding a new chat message in main thread"""
        def add_message():
            self.add_to_chat(message, tag)
